...
class Logger:
    ...
    @classmethod
    def instance(cls, config=None, logFile=None) -> 'Logger':
        """Get or create singleton instance"""
        if cls._instance is None:
            cls._instance = cls()
            cls._instance._initialize(config, logFile)
    ...
    def _initialize(self, config, logFile) -> None:
        """Initialize logger configuration if not already initialized"""
        if config is None:
            return
            
        self.config = config
        self.log_path = os.path.join(self.config.LOG_DIR, logFile)
        self._setup_logger()
    
    def _setup_logger(self) -> None:
        """Configure logging to file and console"""
        self._logger = logging.getLogger('candidate_check')
